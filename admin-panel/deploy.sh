...
# Скрипт развертывания админ-панели SteamTrust
...
SERVER_PATH="/usr/local/src/steamtrust/admin-panel"
...
echo -e "${BLUE}=== Развертывание админ-панели ===${NC}"
...
build_project() {
    ...
    ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "
        ...
        echo 'Запуск сборки проекта (Vite)...'
        npm run build
...
# Перезапуск PM2 процесса админ-панели на сервере
restart_pm2() {
    log "Перезапуск PM2 процесса 'admin-panel'..."
    ssh -i "$SSH_KEY_PATH" -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "bash -lc 'pm2 restart admin-panel && pm2 save'"
    success "PM2 процесс 'admin-panel' перезапущен"
}
...
main() {
    ...
    echo -e "${BLUE}PM2 процесс 'admin-panel' перезапущен${NC}"
}
